...
function progress(over: Partial<InterviewCertificateProgress> = {}): InterviewCertificateProgress {
  ...
  const interviewsRemaining = Math.max(requiredInterviews - completedInterviews, 0);
  return {
...
describe('certificateNextAction', () => {
  it('30. uses SINGULAR wording for one interview remaining', () => {
    const msg = certificateNextAction(progress({ angularExplorerEarned: true, completedInterviews: 4 }));
  ...
  it('31. uses PLURAL wording for multiple interviews remaining', () => {
    const msg = certificateNextAction(progress({ angularExplorerEarned: true, completedInterviews: 3 }));
  ...
  it('points to Angular Explorer when interviews are done but Explorer is locked', () => {
    const msg = certificateNextAction(progress({ angularExplorerEarned: false, completedInterviews: 5 }));
    expect(msg).toBe('Unlock Angular Explorer to earn your certificate.');
  });

  it('guides both when both requirements are incomplete', () => {
    const msg = certificateNextAction(progress({ angularExplorerEarned: false, completedInterviews: 2 }));
  ...
  it('is empty when eligible or already unlocked (no next action)', () => {
    expect(certificateNextAction(progress({ angularExplorerEarned: true, completedInterviews: 5 }))).toBe('');
...
describe('certificateAccessibleSummary', () => {
  it('46. summarises Explorer status, counts, and remaining in one sentence', () => {
    const s = certificateAccessibleSummary(progress({ angularExplorerEarned: true, completedInterviews: 3 }));
  ...
  it('uses singular "1 interview remaining"', () => {
    const s = certificateAccessibleSummary(progress({ angularExplorerEarned: true, completedInterviews: 4 }));
