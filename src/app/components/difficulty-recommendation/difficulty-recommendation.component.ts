      ...
      <section class="dr" aria-labelledby="dr-heading" [class.dr--complete]="r.level === 'complete'">
        @if (r.level === 'complete') {
          @if (certUnlocked()) {
            ...
            <p class="dr__message" i18n>View your Angular Interview Master Certificate.</p>
            <a
              class="dr__btn"
            ...
            @if (certProgress().interviewMasterEarned) {
              <p class="dr__message" i18n>Complete {{ certProgress().requiredInterviews }} qualifying interviews to unlock your Angular Interview Master Certificate.</p>
            } @else {
              <p class="dr__message" i18n>Build a mixed-topic interview and become an Interview Master.</p>
            }
            @if (r.action; as a) {
              <button type="button" class="dr__btn" (click)="act(a)" [attr.aria-label]="a.label">{{ a.label }}</button>
            }
          }
        } @else {
